...
var heartCheck = {
  ...
  start: function () {
    this.timeoutObj = setInterval(function () {
      if (isConnect) {
        console.log('心跳检测向服务端发送消息：' + checkMsg)
        websock.send(checkMsg)
      } else {
        // 重连
        console.log('尝试重连：' + checkMsg)
        reConnect()
      }
    }, this.timeout)
  },
...
function websocketonmessage(e) {
  ...
  let result = JSON.stringify(e.data)

  if (!result) {
    heartCheck.reset()
  } else {
    if (result.msg == 'open success') {
      sessionStorage.setItem('wid', result.wid)
    } else {
      console.log('准备发给前端的消息：' + result)
...
function websocketsend(agentData) {
  console.log('发送给服务端：' + JSON.stringify(agentData))
